...
export default function Community() {
  ...
  const typeIcons: Record<string, any> = { activity: Zap, challenge_complete: Star, collective_join: Heart, story: TrendingUp };

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-black text-foreground flex items-center gap-2"><Zap className="w-6 h-6 text-orange-400" /> CarbonInfluencers</h1>
        <p className="text-muted-foreground text-sm mt-1">Top carbon reducers shaping the movement</p>
      </div>

      {/* Top Influencers */}
      {influencersQuery.data && influencersQuery.data.length > 0 && (
        <div className="card-glass rounded-xl border border-border p-5">
          <h3 className="font-bold text-foreground mb-4 flex items-center gap-2"><Star className="w-4 h-4 text-yellow-400" /> Top Influencers</h3>
          <div className="flex gap-3 overflow-x-auto pb-2">
            {influencersQuery.data.map((inf, idx) => (
              <div key={inf.id} className="flex-shrink-0 flex flex-col items-center gap-2 p-3 rounded-xl border border-border bg-muted/30 min-w-[90px]">
                <div className="relative">
                  <div className="w-12 h-12 rounded-full bg-primary/10 border-2 border-primary/30 flex items-center justify-center text-lg font-bold">
                ...
                <div className="text-xs font-medium text-center truncate w-full">{inf.name ?? "User"}</div>
      ...
      <div className="space-y-3">
        <h3 className="font-bold text-foreground">Live Community Feed</h3>
        {feedQuery.isLoading ? (
          ...
          <div className="card-glass rounded-xl border border-border p-8 text-center">
            ...
            <p className="text-muted-foreground">No activity yet. Start logging to appear here!</p>
          </div>
        ) : (
          feedQuery.data?.map(item => {
            ...
            return (
              <div key={item.id} className={`card-glass rounded-xl border p-4 transition-all ${item.isInfluencer ? "border-yellow-400/30 bg-yellow-400/5" : "border-border"}`}>
                ...
                <div className="flex items-center gap-4 mt-3 pt-3 border-t border-border">
                  ...
                  >
                    <Heart className="w-3.5 h-3.5" /> {item.likeCount ?? (item as any).likesCount ?? 0}
                  </button>
                  {(item as any).influenceScore && <div className="flex items-center gap-1 text-xs text-muted-foreground"><TrendingUp className="w-3 h-3" /> {(item as any).influenceScore} influence</div>}
                </div>
              </div>
            );
          })
