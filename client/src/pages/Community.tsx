...
export default function Community() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-black text-white flex items-center gap-2"><Zap className="w-6 h-6 text-orange-400" /> CarbonInfluencers</h1>
        <p className="text-white/50 text-sm mt-1">Top carbon reducers shaping the movement</p>
      </div>

      {/* Top Influencers */}
      {influencersQuery.data && influencersQuery.data.length > 0 && (
        <div className="card-glass rounded-xl border border-white/10 p-5">
          <h3 className="font-bold text-white mb-4 flex items-center gap-2"><Star className="w-4 h-4 text-yellow-400" /> Top Influencers</h3>
      ...
      <div className="space-y-3">
        ...
        {feedQuery.isLoading ? (
          ...
          <div className="card-glass rounded-xl border border-white/10 p-8 text-center">
            <Zap className="w-10 h-10 text-white/50 mx-auto mb-3" />
            ...
            return (
              <div key={item.id} className={`card-glass rounded-xl border p-4 transition-all ${item.isInfluencer ? "border-yellow-400/30 bg-yellow-400/5" : "border-white/10"}`}>
                ...
                <div className="flex items-center gap-4 mt-3 pt-3 border-t border-white/10">
                  ...
                  {(item as any).influenceScore && <div className="flex items-center gap-1 text-xs text-white/50"><TrendingUp className="w-3 h-3" /> {(item as any).influenceScore} influence</div>}
                </div>
              </div>
            );
          })
