...
export default function Community() {
  ...
  const typeColors: Record<string, string> = { activity: "text-white/70", challenge_complete: "text-white/70", collective_join: "text-white/70", story: "text-white/70" };
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-black text-white flex items-center gap-2"><IconZap className="w-6 h-6 text-white/70" /> CarbonInfluencers</h1>
            ...
            {influencersQuery.data.map((inf, idx) => (
              <div key={inf.id} className="flex-shrink-0 flex flex-col items-center gap-2 p-3 rounded-xl border border-white/10 bg-white/5/30 min-w-[90px]">
                <div className="relative">
                  ...
                  {idx === 0 && <div className="absolute -top-1 -right-1 w-5 h-5 rounded-full bg-white/50 flex items-center justify-center text-[10px]"></div>}
                </div>
              ...
              <div key={item.id} className={`card-glass rounded-xl border p-4 transition-all ${item.isInfluencer ? "border-white/20 bg-white/5" : "border-white/10"}`}>
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {item.carbonKg && <div className="text-xs text-white/70 mt-1">{item.carbonKg.toFixed(2)} kg CO₂</div>}
                  </div>
                </div>
