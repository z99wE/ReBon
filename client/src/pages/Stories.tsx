...
import type { StoryRow } from "@shared/viewModels";

const LOADING_STEPS = [
  { title: "Gathering emissions metrics...", tip: "Every saved kilometer avoids 0.2 kg of CO₂ from entering our atmosphere.", fact: "ReBon Carbon Intelligence acts as your personal greenhouse gas tracker." },
  { title: "Benchmarking against target limits...", tip: "Your Eco Pioneer archetype target helps you stay within safe planetary boundaries.", fact: "Small actions multiply. Reducing household energy by 10% saves up to 500 kg of CO₂ annually." },
  { title: "Evaluating completed challenges...", tip: "Completed challenges directly reduce your carbon balance and build long-term green habits.", fact: "Trees are nature's carbon capture machines, absorbing about 22 kg of CO₂ per year." },
  { title: "Synthesizing custom impact equivalents...", tip: "Translating kilograms of CO₂ into trees and travel equivalents makes your progress tangible.", fact: "ReBon translates raw stats into human-centric narrative chapters." },
  { title: "Running AI narrative engine...", tip: "NVIDIA NIM is writing a personalized, shareable story that highlights your milestones.", fact: "Net Zero is a collective journey. Sharing your story inspires others to start theirs." },
] as const;
...
export default function Stories() {
  ...
  const stories = (storiesQuery.data ?? []) as StoryRow[];
  const activeStory =
    generateMutation.data?.period === period
      ? (generateMutation.data as StoryRow)
      : stories.find((story) => story.period === period) ?? null;

  const handleShare = (story: StoryRow) => {
    if (navigator.share) {
  ...
  const shareToPlatform = (platform: "x" | "linkedin" | "facebook" | "pinterest", story: StoryRow) => {
      ...
      {!isAuthenticated ? (
        ...
        <>
          <div className="card-glass rounded-xl border border-pink-400/20 p-5 space-y-4">
            ...
            <div className="flex gap-2">
              {(["week", "month", "alltime"] as const).map((p) => (
                <button
                  key={p}
                  onClick={() => setPeriod(p)}
                  className={`flex-1 py-2 rounded-lg text-xs font-semibold border transition-all capitalize ${period === p ? "border-pink-400/50 bg-pink-400/10 text-white/70" : "border-white/10 text-white/50 hover:bg-white/5"}`}
                >
            ...
            <button
              onClick={() => generateMutation.mutate({ period })}
            ...
            <div className="card-glass rounded-2xl border border-pink-400/30 p-6 space-y-4 relative overflow-hidden">
              ...
              <div className="grid grid-cols-3 gap-3 pt-2">
                {[
                  ...
                  { icon: IconZap, label: "kg CO₂", value: activeStory.carbonSavedKg.toFixed(1), color: "text-white/70" },
                ].map((stat) => (
              ...
              <div className="pt-4 border-t border-white/5 flex flex-wrap items-center justify-between gap-3 text-[10px] text-white/40 font-mono">
                ...
                <div className="flex gap-1.5">
                  <button onClick={() => shareToPlatform("x", activeStory)} className="px-2.5 py-1 rounded bg-white/5 border border-white/8 hover:bg-white/10 hover:text-white transition-colors" title="Post on X">
                    X / Twitter
                  </button>
                  <button onClick={() => shareToPlatform("linkedin", activeStory)} className="px-2.5 py-1 rounded bg-white/5 border border-white/8 hover:bg-white/10 hover:text-white transition-colors" title="Share on LinkedIn">
                    LinkedIn
                  </button>
                  <button onClick={() => shareToPlatform("facebook", activeStory)} className="px-2.5 py-1 rounded bg-white/5 border border-white/8 hover:bg-white/10 hover:text-white transition-colors" title="Post to Facebook">
                    Facebook
                  </button>
                  <button onClick={() => shareToPlatform("pinterest", activeStory)} className="px-2.5 py-1 rounded bg-white/5 border border-white/8 hover:bg-white/10 hover:text-white transition-colors" title="Pin on Pinterest">
                    Pinterest
                  </button>
                </div>
              </div>
            ...
            !generateMutation.isPending && (
              <div className="card-glass rounded-2xl border border-white/10 p-8 text-center space-y-3">
                ...
                <p className="text-xs text-white/50 max-w-xs mx-auto">
                  Click the "Generate Story" button to let ReBon AI narrate your carbon footprint and challenges for this period.
                </p>
              </div>
            )
          )}
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1">
                      <div className="text-xs text-white/50 mb-1 capitalize">
                        {story.period} · {new Date(story.generatedAt).toLocaleDateString()}
                      </div>
