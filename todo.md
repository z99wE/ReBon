...
- [x] P1: Fix LogActivity.tsx — replace defaultCarbonKg with carbonKg everywhere
- [x] P1: Fix routers.ts logActivity — recompute influence from live DB counts not stale auth snapshot
- [x] P1: Fix db.ts completeChallenge — guard against double-completion (idempotency)
- [x] P1: Fix db.ts joinCollective — guard against duplicate membership rows
- [x] P2: Fix aiRouter.ts — route fast_inference/challenge_generate/coach_response to llama-3.1-8b-instant
